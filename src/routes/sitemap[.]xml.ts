...
function urlEntry(
  loc: string,
  opts?: { lastmod?: string | Date | null; changefreq?: string; priority?: string },
) {
...
${districtUrls
  .map((loc) =>
    urlEntry(loc, { lastmod: now, changefreq: 'daily', priority: '0.8' }),
  )
  .join('\n')}
${propertyEntries
  .map((property) =>
    urlEntry(`${SITE_URL}/property/${property.id}`, {
      lastmod: property.updatedAt ?? now,
      changefreq: 'weekly',
      priority: '0.7',
    }),
  )
